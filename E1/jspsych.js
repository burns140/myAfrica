...
var jsPsych = (function () {
	...
	core.init = function (options) {
		...
		waiting = false;
		loaded = false;
		core.timelineAPI = new TimelineAPI(core)
	...
	core.finishTrial = function (data) {
		...
		if (typeof current_trial.on_finish === 'function') {
			current_trial.on_finish(core.timelineAPI);
		}
	...
	core.getCompletedTrials = function () {
		return timeline
			.getTimelineParameters()
			.timeline
			.slice(0, global_trial_index)
			.map(tn => tn.parameters);
	}

	core.getRemainingTrials = function () {
		return timeline
			.getTimelineParameters()
			.timeline
			.slice(global_trial_index + 1)
			.map(tn => tn.parameters);
	}
	...
	core.removeNodeFromEndOfTimeline = function () {
		if (timeline.length() - 1 <= global_trial_index)
			return undefined
	...
	function TimelineNode(parameters, parent, relativeID) {
		...
		this.getTimelineParameters = function () {
			return timeline_parameters
		}
		...
		// remove trial from end of this timeline
		this.remove = function () {
			return timeline_parameters.timeline.pop();
		}
	...
	class TimelineAPI {
		...
		getCompletedTrials() {
			return this.coreInstance.getCompletedTrials()
		}

		getRemainingTrials() {
			return this.coreInstance.getRemainingTrials()
		}

		getAllData() {
			return jsPsych.data.getData()
		}

		getLastData() {
			return jsPsych.data.getData().slice(-1)[0]
		}

		// This is quite inefficient, but who cares
		setTimeline(timeline) {
			const numTrialsRemaining = this.getRemainingTrials().length
			for (let i = 0; i < numTrialsRemaining; i++) {
				this.popTrial()
			}

			timeline.forEach(trial => this.pushTrial(trial))
		}

		insertTrial(trial, index) {
			...
			const newTimeline = this.getRemainingTrials()
			newTimeline.splice(index, 0, trial)
			this.setTimeline(newTimeline)
		}
		...
		pushTrial(trial) {
			...
			this.coreInstance.addNodeToEndOfTimeline(trial, () => { })
		}

		popTrial() {
			return this.coreInstance.removeNodeFromEndOfTimeline()
		}
	}
	...
	function doTrial(trial) {
		...
		// handle callback at plugin level
		if (typeof current_trial.before_start === 'function') {
			current_trial.before_start(core.timelineAPI);
		}
...
jsPsych.pluginAPI = (function () {
	...
	module.evaluateFunctionParameters = function (trial, protect) {
		...
		var always_protected = ['on_finish', 'before_start'];
